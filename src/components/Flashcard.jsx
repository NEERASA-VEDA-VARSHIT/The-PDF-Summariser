...
function Flashcard() {
  ...
  const handleNext = () => {
    if (counter < state.flashcards.length - 1) {
  ...
  const currentFlashcard = state.flashcards[counter];
    ...
    <div id="flash-cards" className="mt-8 max-w-4xl mx-auto px-4">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">
          Card {counter + 1} of {state.flashcards.length}
        </p>
      </div>
      ...
      <div className="flex justify-center items-center gap-6 mt-12">
        ...
        <button
          className={`px-8 py-3 rounded-lg font-medium transition-all transform hover:scale-105 ${
            counter < state.flashcards.length - 1
              ? "bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg hover:shadow-xl"
              : "bg-gray-200 text-gray-400 cursor-not-allowed"
          }`}
          onClick={handleNext}
          disabled={counter === state.flashcards.length - 1}
        >
          Next
        </button>
      </div>
